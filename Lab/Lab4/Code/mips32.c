...
#endif
                cur->inMemory = 1;
                LocalVariable temp = cur->regNext;
                cur->regNext = NULL;
                cur = temp;
            }else{
                cur = cur->regNext;
            ...
            while(curInterCode != curBasic->end->next){
                ...
                switch(curInterCode->kind){
                    ...
                    case FUNCTION_INTERCODE:{
                        ...
                        // fprintf(fp, "  sw $fp, 0($sp)  in FUNCTION_INTERCODE\n");
                    ...
                    case ASSIGN:{
                        ...
                        if((curInterCode->op1->kind == ADDRESS || curInterCode->op1->kind == TEMPORARY_ADDRESS)
                            ...
                            // fprintf(fp, "  sw %s, 0(%s)  in ASSIGN1\n", regName[index2], regName[index0]);
#ifndef print_lab_4
                            printf("  lw %s, 0(%s)\n", regName[index2], regName[index1]);
                            printf("  sw %s, 0(%s)\n", regName[index2], regName[index0]);
#endif
...
#ifndef print_lab_4
                            printf("  addi %s, %s, %d\n", regName[index0], regName[index2], curInterCode->op1->u.const_value);                               
#endif
                        }else if(curInterCode->op2->kind == CONSTANT){
                            fprintf(fp, "  addi %s, %s, %d\n", regName[index0], regName[index1],
                                                           curInterCode->op2->u.const_value);
#ifndef print_lab_4
                            printf("  addi %s, %s, %d\n", regName[index0], regName[index1], curInterCode->op2->u.const_value);                               
#endif
                        }else{
                            fprintf(fp, "  add %s, %s, %s\n", regName[index0], regName[index1], 
                                                           regName[index2]);
#ifndef print_lab_4
                            printf("  add %s, %s, %s\n", regName[index0], regName[index1], regName[index2]);                               
#endif
                        }
                        findLocalVariable(curInterCode->result, curFunction)->inMemory = 0;
                        break;
                    ...
                    case JE:{
                        ...
                        fprintf(fp, "  beq %s, %s, label%d\n", regName[index1], regName[index2],
                                                           curInterCode->result->u.num);
#ifndef print_lab_4
                        printf("  beq %s, %s, label%d\n", regName[index1], regName[index2], curInterCode->result->u.num);                                   
#endif
                        break;
                    }
                    case JNE:{
                    ...
                    case CALL:{
                        ...
                        fprintf(fp, "  addi $sp, $sp, -4\n");
                        // fprintf(fp, "  sw $ra, 0($sp)  in CALL2\n");
                    ...
                    case READ:{
                        fprintf(fp, "  addi $sp, $sp, -4\n");
                        // fprintf(fp, "  sw $ra, 0($sp)  in READ1\n");
                        fprintf(fp, "  sw $ra, 0($sp)\n");
                        fprintf(fp, "  jal read\n");
                        fprintf(fp, "  lw $ra, 0($sp)\n");
                        fprintf(fp, "  addi $sp, $sp, 4\n");
                    ...
                    case WRITE:{
                        ...
                        fprintf(fp, "  addi $sp, $sp, -4\n");
                        // fprintf(fp, "  sw $ra, 0($sp)  in READ2\n");
                        fprintf(fp, "  sw $ra, 0($sp)\n");
